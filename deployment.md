...
| 2026-08-15 | Least privilege applied | `0010` applied to production with the owner credential, `app_runtime` given a generated alphanumeric password, `DATABASE_URL` repointed at it and the owner string preserved as `MIGRATION_DATABASE_URL`. Alphanumeric on purpose: the password sits inside a URI, and `@ : / ? # [ ]` must be percent-encoded there or the parser reads the password as part of the host and the failure surfaces somewhere unrelated. Verified after redeploy by reading the role back off the running database rather than trusting the migration — `rolsuper`, `rolcreatedb`, `rolcreaterole`, `rolbypassrls` all false; `INSERT, SELECT, UPDATE` on `brews` and no `DELETE`; `SELECT` alone on the vocabularies; no DDL — with `/api/health` green and the log serving, which is what proves the role sufficient as well as restrictive. The owner password was not rotated in the same sitting; it is now read only by `db:apply` and `db:reset`, and STATUS.md carries it as open.                                                                                                                                                                                                                                                                                                                                                                    |
| 2026-08-15 | Rolled back, then fixed | The least-privileged role took the application down without failing anything. `/api/brews` served `{"brews":[],"total":0}` with HTTP 200 and the three `/api/ai/*` routes answered 500, while every privilege read back off the database was exactly as designed. `rolbypassrls: false` had made `0007_rls.sql` apply to the request path for the first time, and its policies assume an authenticated user this product has not built yet: `using (user_id = auth.uid())` matches nothing when every row is unowned and `auth.uid()` is null, and `rate_limit_windows` carried `using (false)` from when only the owner touched it. Restored service by setting `DATABASE_URL` back to the owner string kept in `MIGRATION_DATABASE_URL` and redeploying — no data was ever lost or deleted, only hidden. Migration `0011` is the real fix: policies scoped to `app_runtime` and predicated on `user_id is null`, so v1 works and 0007 still protects any row that gains an owner later. Worth recording because the failure mode is the point — a privilege refuses and row level security filters, so the checks that cleared the first attempt (a green `/api/health`, a `200` from `/api/brews`, six tests asserting refusals) were all true and none was evidence. Read the body, and the `total` inside it, before calling this done. |
