...
| 9     | Security — the ten protection steps           | in progress | 90%      |

**Overall: planning and phases 0 through 6 are complete; Phase 7 is down to its
last artefact; Phase 9's role is built and its fix is under review.** 600 unit
tests, 125 database tests, provider and end-to-end suites passing, `main`
protected and green. **The app is live** — serving from Supabase behind a single
public origin with all three AI surfaces enabled, and the URL recorded in
[deployment.md](./deployment.md). It is running on the **owner** credential
again after the least-privileged role was applied and rolled back the same day:
the privileges were correct, the row level security interaction was not, and the
application served an empty log with HTTP 200 until it was reverted. Migration
`0011` fixes it and is reviewed rather than assumed — see Now. All thirty
acceptance criteria are ticked against evidence; the README and Documentation.md
are level with what shipped.
...
**Phase 9 shipped the least-privileged role, broke the application with it, and
is back on the owner credential while the fix is reviewed.** The sequence is
worth the space, because the failure was silent and the reason it was silent is
the interesting part.

Migration `0010` was applied to production, `app_runtime` was given a generated
password, and `DATABASE_URL` was pointed at it. The role was then read back off
the running database — `rolsuper`, `rolcreatedb`, `rolcreaterole` and
`rolbypassrls` all false, `INSERT, SELECT, UPDATE` on `brews` with no `DELETE`,
`SELECT` alone on the vocabularies, no DDL — and every one of those answers was
correct. The privileges were right. The application was broken anyway.

`rolbypassrls: false` is what did it, and it is the line the previous revision
of this section called "the quiet one" while treating it as unambiguously good
news. It made [`0007_rls.sql`](./supabase/migrations/0007_rls.sql) load-bearing
for the first time — and those policies were written for a product with
authentication, which this one does not have yet:

- `brews_select_own` reads `using (user_id = auth.uid())`. Every v1 row has
  `user_id is null`, `auth.uid()` is null, and `null = null` is null rather than
  true. Every row was filtered out of every read.
- `rate_limit_windows_no_api_access` reads `using (false)`, written when only
  the owner touched that table. The shared AI budget became unwritable, so all
  three `/api/ai/*` routes answered 500.

So the log served empty with **HTTP 200** and nothing failed. Row level security
does not refuse, it filters, and a filtered read is a successful read.

**The verification that missed it was checking status codes rather than
bodies.** `/api/health` was green and `/api/brews` answered 200, both of which
were true and neither of which was evidence. The six negative tests in
`app-runtime-role.db.test.ts` passed for the same reason: they asked whether a
statement was refused, and `select count(*) from brews` is never refused — it
answered zero. A privilege suite cannot see an authorisation bug, because
filtering is not denial.

Migration `0011` is the fix, and it is deliberately not a blanket exemption.
Policies scoped `to app_runtime` and predicated on `user_id is null` restore v1
while leaving 0007 in force for any row that ever gains an owner, so the service
role cannot read another user's brews once authentication ships. The role keeps
every restriction that made it worth having: still no `DELETE` on brews, still
no DDL, still no RLS bypass.

Three tests now assert visibility rather than permission — rows must come back,
a write must land, the rate limit window must be maintainable. Dropping 0011's
policies makes exactly those three fail with `expected +0 to be 12`, which is
the production symptom reproduced in a test, and leaves the original six
passing, which is the proof they never covered it.
...
**Re-apply the least-privileged role, this time with `0011` and with the check
that would have caught the first attempt.** The order matters: apply `0011` to
production with the owner credential, point `DATABASE_URL` back at
`app_runtime`, redeploy, and then read `/api/brews` — the body, and the `total`
inside it, not the status code — before calling it done. A `200` carrying an
empty array is what the first attempt looked like from outside.
...
| 2026-08-15 | The role went live, took the application down quietly, and came back out. Applying `0010` and pointing `DATABASE_URL` at `app_runtime` did exactly what it promised at the privilege layer — every attribute and grant read back correct off the running database — and broke the app at the authorisation layer, because `rolbypassrls: false` finally made `0007_rls.sql` apply to the connection that serves requests. Those policies were written for a product with authentication: `using (user_id = auth.uid())` matches nothing when every row is unowned and `auth.uid()` is null, and `rate_limit_windows` carried a flat `using (false)` from when only the owner touched it. The log served empty with HTTP 200 and the three AI routes answered 500. The verification that cleared it was reading status codes rather than bodies, and the six negative tests missed it because row level security filters instead of refusing — `select count(*) from brews` answered zero and was never denied. Rolled back to the owner credential to restore service, then fixed properly: migration `0011` adds policies scoped to `app_runtime` and predicated on `user_id is null`, so v1 works and 0007 stays in force for any row that ever gains an owner. Three new tests assert visibility rather than permission, and dropping 0011 makes exactly those three fail with `expected +0 to be 12` while the original six still pass. Verified against a real Postgres 17: 122 database tests, then the app itself run as `app_runtime` through create, read, update and soft delete. |
